...
def calculate_threshold_and_features(image: np.ndarray):
    ...
    perimeter = cv2.arcLength(max(contours, key=cv2.contourArea), True) if contours else 0
    fractal_dimension = np.log(area) / np.log(perimeter) if perimeter != 0 else 0
    return threshold, floc_count, np.mean(diameters) if diameters else 0, floc_density, fractal_dimension
        ...
        for filename in os.listdir(input_directory):
            if filename.lower().endswith(tuple(valid_extensions)):
                ...
                if image is not None:
                    threshold, floc_count, avg_diameter, floc_density, fractal_dimension = calculate_threshold_and_features(image)
                    results.append({
                        'Filename': filename,
                        'Threshold': threshold,
...
results_file = '采集图像/第一组处理/result6.xlsx'
